...
class AllmemBerout extends StatefulWidget {
  const AllmemBerout({super.key});

  @override
  State<AllmemBerout> createState() => _AllmemBeroutState();
}

class _AllmemBeroutState extends State<AllmemBerout> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Map<String, Map<String, dynamic>> employeeMaxAttendanceOut = {};
        ...
        for (var record in attendanceList) {
          ...
          } else if (statusApprove == "Out") {
            if (!employeeMaxAttendanceOut.containsKey(name) ||
                attendanceId >
                    employeeMaxAttendanceOut[name]!['attendance_id']) {
              employeeMaxAttendanceOut[name] = record;
            }
          }
        }

        // คัดกรองข้อมูลที่ต้องการ
        List<Map<String, dynamic>> filteredPending = employeeMaxAttendanceOut
            .values
            .where((record) => record["status"]?.toString().trim() == "Pending")
            .toList();

        List<Map<String, dynamic>> filteredApprove = employeeMaxAttendanceOut
            .values
            .where((record) => record["status"]?.toString().trim() == "Approve")
            .toList();

        List<Map<String, dynamic>> filteredReject = employeeMaxAttendanceOut
            .values
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: CircleAvatar(
                            foregroundImage:
                                MemoryImage(base64Decode(item["picture_out"])),
                            radius: MediaQuery.of(context).size.width * 0.07),
                        title: Text('${item["employee_name"]}',
                            style:
                                const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle: Text(
                          'เวลาออกงาน: $formattedDate',
                        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('การออกงาน', style: TextStyle(color: Colors.white)),
        centerTitle: true,
